...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
	  ...
	  //   double next_s = car_s+(i+1)*dest_inc;
	  //   double next_d = 6;
	  ...
	  for( int i = 0; i < sensor_fusion.size(); i++ ){
	    ...
	    if ( d < (2+4*lane+2) && d > (2+4*lane-2) ){
	      //keep safe distace appropriate for current vehcle speed
	      if ( (check_car_s > car_s ) && ((check_car_s-car_s) < target_speed ) ){
		target_speed -= .224;
	      }
	    }
	  }
	  ...
	  //	  typedef e::Spline<double,2> Spline2d;
	  tk::spline s;
